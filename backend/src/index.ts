...
async function main() {
    ...
    (
      ...
      if (isPgError(err)) {
        if (err.code === "42P01") {
          res.status(503).json({
            error:
              "Database schema is incomplete. From the backend folder run: npm run db:schema",
          });
          return;
        }
  ...
  const server = app.listen(port, host, () => {
    ...
    if (env.NODE_ENV === "production" && !env.APP_BASE_URL) {
      console.warn(
        "APP_BASE_URL was not set and no deployment public URL was detected. Set APP_BASE_URL to the real public URL for email links and external access.",
      );
    }
  });
