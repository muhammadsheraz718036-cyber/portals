...
export function AdminRoles() {
  ...
  const openCreate = () => {
    setEditRole(null); setName(""); setDescription(""); setPermissions([]);
    setDialogOpen(true);
  };

  const openEdit = (role: Role) => {
    setEditRole(role); setName(role.name); setDescription(role.description); setPermissions(role.permissions);
    setDialogOpen(true);
  };
    ...
    <div className="space-y-4">
                    ...
                    <label key={perm.id} className="flex items-start gap-2 cursor-pointer">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          setPermissions(checked ? [...permissions, perm.id] : permissions.filter(p => p !== perm.id));
                        }}
                        className="mt-0.5"
                      />
                      <div>
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          ...
          {roles.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No roles yet.</p>}
        </div>
      )}
    </div>
  );
