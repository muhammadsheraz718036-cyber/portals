...
export default function AuditLogs() {
    ...
    return (
      <div className="p-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Only administrators can view audit logs.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Audit Logs</h1>
        <p className="text-sm text-muted-foreground mt-1">Complete activity history — who did what and when</p>
      </div>
      ...
      <Card className="border">
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/30">
                  <th className="text-left px-4 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Timestamp</th>
                  <th className="text-left px-4 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">User</th>
                  <th className="text-left px-4 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Action</th>
                  <th className="text-left px-4 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Target</th>
                  <th className="text-left px-4 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Details</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {filtered.map((log) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-primary font-medium">{log.target}</td>
            ...
            {filtered.length === 0 && (
              <div className="px-4 py-12 text-center text-muted-foreground text-sm">No audit entries found</div>
            )}
          </div>
        </CardContent>
      </Card>
